    ...
    for cls_ind in range(cls_num): 
        ...
        for frame_ind in range(frame_num-1): 
            ...
            for box1_ind,box1 in enumerate(dets1):
                ...
                links_box=[ovr_ind for ovr_ind,ovr in enumerate(ovrs) if ovr >= IOU_THRESH] #box box link
...
def findMaxPath(links,dets):
    ...
    for link_ind,link in enumerate(links[::-1]): #link，list
        curmaxpaths=[]
        linkflags=np.zeros(len(maxpaths[0]),int)
        det_ind=len(links)-link_ind-1
        for ind,linkboxes in enumerate(link): #box,ink，list
            if linkboxes == []:
...
def deleteLink(dets,links, rootindex, maxpath,thesh):
    for i,box_ind in enumerate(maxpath):
        ...
        deletes=[ovr_ind for ovr_ind,ovr in enumerate(ovrs) if ovr >= IOU_THRESH] #box index
        if rootindex+i<len(links): #box_ind box link
            for delete_ind in deletes:
                links[rootindex+i][delete_ind]=[]
        if i > 0 or rootindex>0:
            for priorbox in links[rootindex+i-1]: #box_ind link
                for delete_ind in deletes:
